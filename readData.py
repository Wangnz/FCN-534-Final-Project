...
# AP_list = {
#     'H1': {'7a:f0', '7a:f1', '7a:f3'},
#     'H2': {'77:10', '77:11', '77:13'},
#     'H3': {'fc:90', 'fc:92', 'fc:93'},
#     'H4': {'77:70', '77:71', '77:73'},
#     'L1': {'85:f0', '85:f2', '85:f3'},
#     'L2': {'7c:10', '7c:11', '7c:13'},
#     'P1': {'7a:d0', '7a:d1', '7a:d3'},
#     'P2': {'7b:b0', '7b:b1', '7b:b3'},
#     'S1': {'85:b0', '85:b1', '85:b3'},
#     'S2': {'e1:70', 'e1:72', 'e1:73'},
#     'android2657': {'c0:f9'}
# }
...
def get_access_points():
    ...
    filtered_ap_list = []
    for index in xrange(1, len(raw_data)):
        signal_data = {}
        raw_ap = raw_data[index].strip().split(' ')
        
        if len(raw_ap) < 3:
            continue;

        # excluding signals that are not useful (e.g. printers)
        if raw_ap[0].startswith('cs_') or raw_ap[0] == 'android2657':
            group_res = get_ap_group(AP_list, raw_ap[1][-5:])
            if group_res == 'X': # ignore unknow devices
                continue
            print raw_ap[0]
            signal_data['SSID'] = raw_ap[0]
            signal_data['BSSID'] = raw_ap[1][-5:].upper()
            signal_data['RSSI'] = float(raw_ap[2])
            signal_data['GROUP'] = group_res

            filtered_ap_list.append(signal_data)

    # sort by signal strength (RSSI)
    sorted_ap_list = sorted(filtered_ap_list, key=lambda e: e['RSSI'], reverse=True)
